...
$("#validationName").keyup(function () {
  if (
    ...
    $("#name-feed").css("class", "valid-feedback text-success");
    nameValidated = true;
  } else {
    $("#name-feed").show();
    $("#name-feed").text("Invalid name! Check your input");
    $("#name-feed").css("class", "invalid-feedback text-warning");
    nameValidated = false;
  }
...
$("#validationPhone").keyup(function (e) {
  if ($("#validationPhone").val().match(/^\d+$/)) {
    ...
    $("#phone-feed").css("class", "valid-feedback text-success");
    phoneValidated = true;
  }
  if ($("#validationPhone").val().length < 6) {
    $("#phone-feed").show();
    $("#phone-feed").text("Phone number should have at least 6 digits!");
    $("#phone-feed").css("class", "invalid-feedback text-warning");
  ...
  if (
    ...
    $.ajax({
      ...
      success: function (response) {
        alert("Form submitted successfully. Will keep in touch soon!!!");
